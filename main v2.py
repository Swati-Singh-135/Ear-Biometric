...
def dfs(img,visited,i,j,sign=1):
...
def createNormals(outerEdge, points, sign=1):
    x1 , y1 = points[0]
    x2 , y2 = points[1]
    m = (y2-y1)/(x2-x1)
    m = -1/m
...
def getLmin2(img, umax,midPoint,outerEdge,sign=1):
    x1 , y1 = umax
    x2 , y2 = midPoint
    m = (y2-y1)/(x2-x1)
...
print(lmin2)
cv2.circle(img2, lmin2, 2, (0,255,255), 2)
